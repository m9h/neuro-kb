...
Baseline rows are frozen reference checkpoints evaluated with the same probe suite. They help calibrate the plot, but pathology-specific baselines are not valid initialization points for nanopath leaderboard submissions. The reference scripts live in `baselines/`. The broader pathology FM landscape (Virchow / Virchow2, GigaPath / Prov-GigaPath, Phikon / Phikon-v2, Hibou, CONCH, RudolfV, CTransPath) is summarized in `NOTES.md` alongside internal recipe ablations on the Modal Volume that are not yet on Labless.
